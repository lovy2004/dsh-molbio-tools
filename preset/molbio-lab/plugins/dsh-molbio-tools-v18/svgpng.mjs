 ...
 * (font-size, font-weight, text-anchor, dominant-baseline, textLength,
 * lengthAdjust, rotate transform), `fill`/`stroke` hex colours, `fill-opacity`,
 * `stroke-width`, `stroke-dasharray`, `stroke-linecap`. Everything else —
 * gradients, clip paths, filters, images, other transforms — is *counted* and
 * reported in `unsupported` rather than silently dropped, so a test can assert
 * that the real renderers stay inside the subset.
 *
...
/** Advance width of one glyph, in grid units. */
const GLYPH_ADVANCE = 6;
/** Grid units per em: cap height is 7 units, so 10 units = 1 em (0.7 em caps). */
const FONT_UNITS_PER_EM = 10;
 ...
 * both sufficient and simpler than a full XML tree.
 */
function parseSvgDocument(svg) {
  ...
  while ((match = tagPattern.exec(svg)) !== null) {
    ...
    if (closing) {
      if (pendingText !== undefined && pendingText.name === name) {
        if (pendingText.attributes !== undefined) {
          elements.push({ name: 'text', attributes: pendingText.attributes, text: decodeEntities(svg.slice(pendingText.contentStart, match.index)) });
        }
    ...
    if (pendingText !== undefined && pendingText.attributes !== undefined && pendingText.name === 'text') {
      // A nested tag inside <text> (e.g. <tspan>) — close the run we captured.
      elements.push({ name: 'text', attributes: pendingText.attributes, text: decodeEntities(svg.slice(pendingText.contentStart, match.index)) });
      pendingText = undefined;
    }
    ...
    if (name === 'text') {
      pendingText = { name, attributes, contentStart: tagPattern.lastIndex };
      continue;
    }
    if (name === 'g') {
...
function drawElement(context, element) {
  ...
  switch (element.name) {
    ...
    case 'text': {
      const fontSize = finiteOr(attributes['font-size'], 16);
      const textLength = attributes.textLength === undefined ? undefined : finiteOr(attributes.textLength, undefined);
      const layout = layoutText(element.text ?? '', scale, {
        x: number('x'),
        y: number('y'),
        fontSize,
        anchor: attributes['text-anchor'],
        baseline: attributes['dominant-baseline'],
        textLength,
      });
      ...
      strokeSubpaths(canvas, subpaths, { color: paint.fill, alpha: paint.fillAlpha, width: layout.strokeWidth, scale, linecap: 'round' });
      return;
    }
    default:
