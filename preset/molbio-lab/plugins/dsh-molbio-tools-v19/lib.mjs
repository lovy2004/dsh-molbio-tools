 ...
 * exactly that).
 */
export const CODON_TABLE_BY_NAME = CODON_TABLE;
 ...
 */
export const METHYLATION_SITES = {
  dam: {
    site: 'GATC',
 ...
 */
export const ANY_BUFFER_ENZYMES = ['BamHI', 'BglII', 'EcoRI', 'HindIII', 'NdeI', 'NheI', 'PstI', 'SacI', 'SalI', 'ScaI', 'SpeI', 'XbaI', 'XhoI'];

/** Enzymes whose activity the table covers (plus {@link ANY_BUFFER_ENZYMES}). */
export const ENZYME_BUFFERS = {
  AgeI: ['r1.1', 'cutsmart'],
  ApaI: ['cutsmart'],
  AscI: ['r3.1', 'cutsmart'],
  AvrII: ['r1.1', 'cutsmart'],
...
export function sharedBuffers(names) {
...
export function enzymesMissingBufferData(names) {
 ...
 */
export function methylationSites(seq, marks = Object.keys(METHYLATION_SITES)) {
  const out = [];
  const rc = reverseComplement(seq);
 ...
 */
export function methylationImpact(seq, names, { marks = ['dam', 'dcm'] } = {}) {
 ...
 */
export function doubleDigest(seq, first, second, circular) {
