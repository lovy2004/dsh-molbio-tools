 ...
 * `line`, `circle`, `polygon`, `polyline`, `path`, `text`, hex colours, plain
 * numeric attributes. No `<g>`, no `transform`, no `url(#…)`, no gradients,
 * no CSS classes. That is not a style preference: `test/svgpng.mjs` asserts on
 * REAL renderer output that `unsupported` and `missing_glyphs` are both EMPTY,
 * and `attach_image` hands those pixels to the model. A new construct must
 * therefore fail in the test suite rather than vanish from the picture.
 *
