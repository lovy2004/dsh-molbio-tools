...
const helicalWheelTool = (ctx) => define({
  ...
  async execute(args, exec) {
    ...
    const { file, viewed } = await writeSvgFile(ctx, exec, args, svg, 'helical-wheel.svg');
    const classCounts = {
...
const hydropathyTool = (ctx) => define({
  ...
  async execute(args, exec) {
    ...
    const { file, viewed } = await writeSvgFile(ctx, exec, args, svg, 'hydropathy.svg');
    return { ...profile, svg_path: file, auto_viewed: viewed };
  },
});
...
const sequenceLogoTool = (ctx) => define({
  ...
  render(value) {
    return [
      `sequence logo written to ${value.svg_path} (${value.sequence_count} sequences x ${value.columns} columns, ${value.score_type}${value.small_sample && value.score_type === 'bits' ? ', small-sample corrected' : ''}${value.auto_viewed ? ', opened in the default viewer' : ''})`,
  ...
  async execute(args, exec) {
    ...
    const { file, viewed } = await writeSvgFile(ctx, exec, args, svg, 'logo.svg');
...
const grnaDesignTool = (ctx) => define({
  ...
  async execute(args, exec) {
    ...
    if (args.map_path !== undefined && args.map_path !== '') {
      ...
      const written = await writeSvgFile(ctx, exec, { output_path: args.map_path, auto_view: args.auto_view }, svg, 'grna-map.svg');
