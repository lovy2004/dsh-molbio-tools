...
const msaAlignTool = (ctx) => define({
  ...
  description: 'Progressive multiple sequence alignment of 2-50 IUPAC DNA sequences (up to 3000 bases each, 30000 bases total). Pairwise/profile global alignment uses affine gaps with free terminal gaps (match +4 / mismatch -4 / gap open -6 / extend -2); the merge order comes from a UPGMA tree over 5-mer distances and profiles are aligned with sum-of-pairs scoring ("once a gap, always a gap"). FREE END GAPS NEVER DROP A RESIDUE: when the best-scoring path stops before the end of a sequence, the residues past that point are emitted as overhang columns, so every output row ungapped equals its input sequence (shorter sequences simply show trailing gaps). Returns the aligned sequences in input order plus pairwise identity statistics. Provide either `sequences` (array of strings) or `fasta_path` (workspace FASTA); pass `save_path` to also write the aligned FASTA into the workspace. Deterministic heuristic — a working alignment for comparison, not phylogenetic ground truth.',
  parameters: {
    type: 'object',
    properties: {
...
const phylogeneticTreeTool = (ctx) => define({
  ...
  description: 'Build a distance-based phylogenetic tree from sequences: corrected distances (p-distance / Jukes-Cantor / Kimura 2P / Tamura-Nei), UPGMA or neighbour-joining, bootstrap support with an explicit seed, a strict or majority-rule consensus, Newick I/O, and an SVG tree (rectangular, circular or fan layout; long leaf names wrap onto multiple lines in the rectangular layout rather than overlapping). Raw sequences are aligned first with the same progressive aligner as molbio_msa_align, which places every supplied residue. This is a DISTANCE method, not maximum likelihood: bootstrap percentages are resampling support, NOT p-values, and the output says so.',
  parameters: {
    type: 'object',
    properties: {
  ...
  async execute(args, exec) {
    ...
    if (!alreadyAligned) {
      if (args.aligned !== false) {
        ...
        // shortened row pass unnoticed. v20 fixed the aligner's trailing
        // overhang (`coverageShortfall` lives in phylo.mjs so the test can
        // drive the guard directly).
        const lost = coverageShortfall(rows, entries);
        if (lost.length > 0) {
