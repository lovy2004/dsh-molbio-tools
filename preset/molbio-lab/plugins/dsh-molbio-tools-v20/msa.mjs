 ...
 * - FREE END GAPS NEVER DROP A RESIDUE. The chosen path may stop short of
 *   either profile's end (that is what "free end gap" means, and it can be the
 *   higher-scoring choice), so the residues past the endpoint are emitted as
 *   pure overhang columns rather than discarded. Before v20 the trailing
 *   overhang was silently truncated: 11 bp aligned against 10 bp returned 10
 *   columns and lost a base. `test/smoke.mjs` asserts the invariant directly —
 *   the ungapped characters of every output row equal its input sequence.
 *   The reported `score` is still the DP optimum, because an overhang column
 *   scores 0, exactly like the terminal gap it replaces.
...
function alignProfiles(profileA, profileB) {
  ...
  // The path is expected to run all the way to a boundary (i === 0 or
  // j === 0), so the loop condition tests "input exhausted", not merely the
  // cursor value: the tail below must run for any cursor that is NOT 0.
  const opA = [];
  const opB = [];
  let i = bi;
  let j = bj;
  ...
  // Residues BEFORE the path's start are a leading overhang: a free-end
  // alignment is free to stop short of either profile's first column. These
  // are not gaps the alignment chose — they are input residues the chosen
  // path never visited — so they are padded as a pure overhang.
  while (i > 0) {
    opA.push(columnString(profileA.rows, i - 1));
    opB.push('-'.repeat(wb));
    i--;
  ...
  // Residues AFTER the path's end are the trailing overhang, and this is where
  // a longer sequence used to be SILENTLY TRUNCATED: with free end gaps the
  // best score may be reached short of the corner (11 bp vs 10 bp scores 40 at
  // (10, 10) but only 30 at (11, 10)), and the residues past the endpoint were
  // simply never emitted.
  //
  // Collected in left-to-right order in its own list (rather than pushed onto
  // opA/opB, which hold reversed columns), so the single reverse below cannot
  // scramble it. Appending keeps every input residue in the output; the score
  // above is unchanged because an overhang column scores 0, exactly like the
  // terminal gap it replaces.
  const suffixA = [];
  const suffixB = [];
  for (let k = bi; k < la; k++) {
    suffixA.push(columnString(profileA.rows, k));
    suffixB.push('-'.repeat(wb));
  }
  for (let k = bj; k < lb; k++) {
    suffixA.push('-'.repeat(wa));
    suffixB.push(columnString(profileB.rows, k));
  }
  opA.reverse();
  opB.reverse();
  opA.push(...suffixA);
